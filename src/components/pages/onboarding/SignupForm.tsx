...
import { User, Lock, Mail, Type, ArrowRight, Loader, Building2, Sparkles } from 'lucide-react';
import { motion } from 'framer-motion';
import { Link } from 'react-router-dom';

interface SignupFormProps {
    onSuccess: (userId: any, email: string, accountType: string) => void;
}

const SignupForm = ({ onSuccess }: SignupFormProps) => {
    const [formData, setFormData] = useState({ username: '', password: '', email: '', fullName: '', accountType: '' });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.username || !formData.password || !formData.email || !formData.fullName || !formData.accountType) {
        ...
        try {
            ...
            if (res.ok && data.userId) {
                onSuccess(data.userId, formData.email, formData.accountType);
            } else {
        ...
        >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                    <label className="text-xs font-semibold uppercase tracking-wider text-gray-400 ml-1">I am a</label>
                    <div className="grid grid-cols-2 gap-3">
                        <button
                            type="button"
                            onClick={() => setFormData({ ...formData, accountType: 'influencer' })}
                        ...
                        >
                            <Sparkles className={`w-6 h-6 ${formData.accountType === 'influencer' ? 'text-brand-primary' : ''}`} />
                            <span className="font-semibold">Creator / Influencer</span>
                            <span className="text-xs opacity-70">Grow your audience</span>
                        </button>
                        <button
                            type="button"
                        ...
                        >
                            <Building2 className={`w-6 h-6 ${formData.accountType === 'business' ? 'text-purple-500' : ''}`} />
                            <span className="font-semibold">Business / Brand</span>
                            <span className="text-xs opacity-70">Find influencers</span>
                        </button>
                    </div>
                </div>
