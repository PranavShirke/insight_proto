...
import { User, Mail, Bell, Globe, Instagram, Twitter, Facebook, Save, Youtube, Building2, Sparkles, RefreshCw } from 'lucide-react';
...
const Settings = () => {
    ...
    const handleAccountTypeSwitch = async (newType: 'influencer' | 'business') => {
        if (user?.accountType === newType) return;
        ...
        try {
            ...
            if (response.ok) {
                ...
                await checkAuth();
                setSaveMessage(`✓ Switched to ${newType} account!`);
            } else {
                const errorText = await response.text();
                console.error('Account type switch failed:', errorText);
                setSaveMessage('Failed to switch account type');
            }
        } catch (error) {
        ...
        <div className="max-w-4xl mx-auto space-y-8 pb-12">
            ...
            <section className="bg-dark-surface border border-white/5 rounded-2xl p-6 md:p-8">
                <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-white">Account Type</h2>
                        <p className="text-sm text-dark-muted">Switch between influencer and business views</p>
                    </div>
                </div>

                <div className="flex flex-col sm:flex-row gap-4">
                    <button
                        onClick={() => handleAccountTypeSwitch('influencer')}
                        disabled={isSwitchingAccount}
                        className={`flex-1 flex items-center justify-center gap-3 px-6 py-4 rounded-xl border-2 transition-all ${
                            user?.accountType === 'influencer' || !user?.accountType
                                ? 'border-brand-primary bg-brand-primary/20 text-white'
                                : 'border-white/10 bg-dark-bg text-dark-muted hover:border-brand-primary/50'
                        }`}
                    >
                        <Sparkles className="w-5 h-5" />
                        <div className="text-left">
                            <div className="font-semibold">Influencer</div>
                            <div className="text-xs opacity-70">Manage your content & analytics</div>
                        </div>
                        {(user?.accountType === 'influencer' || !user?.accountType) && (
                            <span className="ml-auto text-xs bg-brand-primary px-2 py-1 rounded-full">Active</span>
                        )}
                    </button>
                    ...
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-3 px-6 py-4 rounded-xl border-2 transition-all ${
                            user?.accountType === 'business'
                                ...
                                : 'border-white/10 bg-dark-bg text-dark-muted hover:border-purple-500/50'
                        }`}
                    >
                        <Building2 className="w-5 h-5" />
                        <div className="text-left">
                            <div className="font-semibold">Business</div>
                            <div className="text-xs opacity-70">Find influencers & run campaigns</div>
                        </div>
                        {user?.accountType === 'business' && (
                            <span className="ml-auto text-xs bg-purple-500 px-2 py-1 rounded-full">Active</span>
                        )}
                    </button>
                </div>

                {isSwitchingAccount && (
                    <div className="mt-4 text-center text-dark-muted">
