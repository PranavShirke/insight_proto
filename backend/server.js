...
import { initDB, User, SocialAccount, Campaign } from './database.js';
...
app.post('/auth/login', passport.authenticate('local'), async (req, res) => {
    ...
    res.json({
        ...
        accountType: req.user.accountType || 'influencer',
        businessInfo: req.user.accountType === 'business' ? {
            industry: req.user.businessIndustry,
            size: req.user.businessSize,
            budget: req.user.businessBudget,
            targetCategories: req.user.targetCategories,
            targetRegions: req.user.targetRegions
        } : null,
        connections
    });
});
...
app.post('/auth/signup', async (req, res, next) => {
    try {
        const { username, password, email, fullName, accountType } = req.body;
        ...
        const user = await User.create({
            ...
            accountType: accountType || 'influencer', // Default to influencer
        });
...
app.post('/api/user/onboarding', isAuthenticated, async (req, res) => {
    try {
        const { answers, accountType } = req.body;
        ...
        // Update account type if provided (business or influencer)
        if (accountType) {
            user.accountType = accountType;
        }
        
        // If business, store additional business fields
        if (accountType === 'business' && answers) {
            user.businessIndustry = answers.industry || null;
            user.businessSize = answers.companySize || null;
            user.businessBudget = answers.marketingBudget || null;
            user.targetCategories = answers.targetCategories || [];
            user.targetRegions = answers.targetRegions || [];
        }
...
app.put('/api/user/settings', isAuthenticated, async (req, res) => {
    try {
        const { fullName, preferences, accountType, businessInfo } = req.body;

        // Build update object
        const updateData = {
            fullName: fullName || req.user.fullName,
            preferences: preferences ? JSON.stringify(preferences) : req.user.preferences
        };

        // Allow updating account type
        if (accountType && ['influencer', 'business'].includes(accountType)) {
            updateData.accountType = accountType;
        }

        // Update business info if switching to business
        if (businessInfo && accountType === 'business') {
            updateData.businessIndustry = businessInfo.industry;
            updateData.businessSize = businessInfo.size;
            updateData.businessBudget = businessInfo.budget;
            updateData.targetCategories = businessInfo.targetCategories;
            updateData.targetRegions = businessInfo.targetRegions;
        }
...
// PUT endpoint to switch account type
app.put('/api/user/account-type', isAuthenticated, async (req, res) => {
    try {
        const { accountType, businessInfo } = req.body;

        if (!['influencer', 'business'].includes(accountType)) {
        ...
        res.json({
            success: true,
            message: `Account type switched to ${accountType}`,
            accountType: req.user.accountType,
            businessInfo: accountType === 'business' ? {
                industry: req.user.businessIndustry,
                size: req.user.businessSize,
                budget: req.user.businessBudget,
                targetCategories: req.user.targetCategories,
...
// ==========================================
// Business User API Endpoints
// ==========================================

// Get all campaigns for the authenticated business user
app.get('/api/business/campaigns', isAuthenticated, async (req, res) => {
    try {
...
app.post('/api/business/campaigns', isAuthenticated, async (req, res) => {
    try {
        if (req.user.accountType !== 'business') {
            return res.status(403).json({ error: 'Business account required' });
        }

        const { name, budget, startDate, endDate, goals, targetCategories, targetRegions } = req.body;
        ...
        const campaign = await Campaign.create({
            ...
            targetCategories: JSON.stringify(targetCategories || []),
...
// Update campaign
app.put('/api/business/campaigns/:id', isAuthenticated, async (req, res) => {
    try {
        if (req.user.accountType !== 'business') {
            return res.status(403).json({ error: 'Business account required' });
        }
        ...
        const campaign = await Campaign.findOne({
            where: { id: req.params.id, businessId: req.user.id }
        });
        ...
        const { name, budget, status, startDate, endDate, goals, influencers, deliverables, metrics } = req.body;
        ...
        await campaign.update({
            ...
            goals: goals !== undefined ? JSON.stringify(goals) : campaign.goals,
            influencers: influencers !== undefined ? JSON.stringify(influencers) : campaign.influencers,
            deliverables: deliverables !== undefined ? JSON.stringify(deliverables) : campaign.deliverables,
            metrics: metrics !== undefined ? JSON.stringify(metrics) : campaign.metrics
        });
...
// Delete campaign
app.delete('/api/business/campaigns/:id', isAuthenticated, async (req, res) => {
    try {
        if (req.user.accountType !== 'business') {
            return res.status(403).json({ error: 'Business account required' });
        }
...
// AI Matching endpoint - returns matched influencers based on requirements
app.post('/api/business/match', isAuthenticated, async (req, res) => {
    try {
        if (req.user.accountType !== 'business') {
            return res.status(403).json({ error: 'Business account required' });
        }

        const { campaignGoal, targetCategory, budget, platform, contentType, audienceSize } = req.body;

        // Generate mock AI matching results (in production, this would query a real database)
        const mockInfluencers = [
            { id: 1, name: 'Tech Reviewer Pro', matchScore: 96, category: 'Tech', followers: 2500000, engagement: 4.2 },
            { id: 2, name: 'Digital Lifestyle', matchScore: 92, category: 'Technology', followers: 1800000, engagement: 5.1 },
            { id: 3, name: 'Gadget Guru', matchScore: 89, category: 'Tech', followers: 3200000, engagement: 3.8 },
            { id: 4, name: 'Innovation Hub', matchScore: 87, category: 'Tech', followers: 950000, engagement: 6.2 },
            { id: 5, name: 'Future Forward', matchScore: 85, category: 'Business', followers: 1200000, engagement: 4.5 }
        ];

        res.json({ matches: mockInfluencers });
    } catch (e) {
...
app.get('/api/business/predictions/:influencerId', isAuthenticated, async (req, res) => {
    try {
        if (req.user.accountType !== 'business') {
            return res.status(403).json({ error: 'Business account required' });
        }

        // Generate mock prediction data (in production, this would use ML models)
        ...
        // Generate 36 months of historical data
        for (let i = 0; i < 36; i++) {
            const date = new Date();
            ...
            historicalData.push({
                ...
                followers: Math.round(followers * (0.9 + Math.random() * 0.2)),
                likes: Math.round(likes * (0.9 + Math.random() * 0.2)),
                engagement: engagement * (0.9 + Math.random() * 0.2)
            });
            
            followers *= 1.025; // 2.5% monthly growth
            likes *= 1.02;
        }
        
        // Generate 9 months of predictions
        for (let i = 0; i < 9; i++) {
            const date = new Date();
            ...
            predictionData.push({
                ...
                engagement: engagement,
                confidence: 0.92 - (i * 0.02) // confidence decreases over time
            });
...
app.get('/api/business/dashboard', isAuthenticated, async (req, res) => {
    try {
        if (req.user.accountType !== 'business') {
            return res.status(403).json({ error: 'Business account required' });
        }
        ...
        const stats = {
            ...
            totalInfluencers: campaigns.reduce((sum, c) => {
                const influencers = JSON.parse(c.influencers || '[]');
                return sum + influencers.length;
            }, 0)
        };
